...
safe_command.run(subprocess.run, ["mkdir", "-p", folder_CDS])
safe_command.run(subprocess.run, ["mkdir", "-p", f"{folder_CDS}/01_unalign"])
safe_command.run(subprocess.run, ["mkdir", "-p", f"{folder_CDS}/02_aligned"])
safe_command.run(subprocess.run, ["mkdir", "-p", f"{folder_CDS}/02_aligned/fasta"])
safe_command.run(subprocess.run, ["mkdir", "-p", f"{folder_CDS}/02_aligned/nexus"])

# Step 4: Run your Python script using the full_script_path
safe_command.run(subprocess.run, ["python", full_script_path, "-i", ".", "-o", folder_CDS, "-s", "cds", "-t", "5", "-l", "6"])
