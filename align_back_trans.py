...
def check_trans(identifier, nuc, prot, table):
    ...
    if len(nuc) % 3:
        sys.exit(f"Nucleotide sequence for {identifier} is length {len(nuc)} (not a multiple of three)")
    ...
    if len(t) != len(p):
        err = (f"Inconsistent lengths for {identifier}, ungapped protein {len(p)}, "
...
def sequence_back_translate(aligned_protein_record, unaligned_nucleotide_record, gap, table=0):
    ...
    if table:
        ...
        sys.exit(f"Inconsistent lengths for {aligned_protein_record.id}, ungapped protein {len(ungapped_protein)}, "
    ...
    for amino_acid in aligned_protein_record.seq:
        if amino_acid == gap:
            ...
            seq.append(nuc[:3])
            nuc = nuc[3:]
    assert not nuc, f"Nucleotide sequence for {unaligned_nucleotide_record.id} longer than protein {aligned_protein_record.id}"
...
def alignment_back_translate(protein_alignment, nucleotide_records, key_function=None, gap=None, table=0):
    ...
    for protein in protein_alignment:
        try:
            ...
            raise ValueError(f"Could not find nucleotide sequence for protein {protein.id}")
